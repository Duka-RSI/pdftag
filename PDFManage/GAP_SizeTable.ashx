...
public class Passport : IHttpHandler, IRequiresSessionState
{
    ...
    public void delete(HttpContext context)
    {
        ...
        sSql = "delete  PDFTAG.dbo.GAP_SizeTable  where lustid=@lustid  ";
...
(luhid,rowid,codeid,Name,Criticality,TolA,TolB,HTMInstruction,lusthid,A1,A2,A3,A4,A5,A6,A7,A8,A9,A10,org_lustid) 
 select luhid,rowid,codeid,Name,Criticality,TolA,TolB,HTMInstruction,lusthid,A1,A2,A3,A4,A5,A6,A7,A8,A9,A10, org_lustid
   from PDFTAG.dbo.GAP_SizeTable where lustid=@lustid;";
    ...
    public void get_LearnmgrItem(HttpContext context)
    {
        ...
        sSql = "select " + col + " as TextOrg  from  PDFTAG.dbo.GAP_SizeTable where lustid=@lustid";
    ...
    public void saveByCol(HttpContext context)
    {
        ...
        using (var cn = SqlMapperUtil.GetOpenConnection("DB"))
        {
            ...
            if (col.StartsWith("A"))
            {
                sSql = "select H" + (col.Replace("A", "")) + " as TextOrg  from  PDFTAG.dbo.GAP_SizeTable_Header where lusthid=@lusthid";
                ...
                //sSql = "select lusthid  from  PDFTAG.dbo.GAP_SizeTable where lustid_relation=@lustid";
                //var resLu_SizeTable_1_Org = cn.Query<Text>(sSql, new { lustid = lustid_org }).FirstOrDefault();

                //sSql = "select *  from  PDFTAG.dbo.GAP_SizeTable_Header  where lusthid=@lusthid";
            ...
            //sSql = "update PDFTAG.dbo.GAP_SizeTable_1 \n";
    ...
    public void getContent(HttpContext context)
    {
        ...
        using (System.Data.SqlClient.SqlCommand cm = new System.Data.SqlClient.SqlCommand(sSql, sql.getDbcn()))
        {
            ...
            sSql += "from PDFTAG.dbo.GAP_Header a              \n";
            sSql += " where 1=1   \n";
            sSql += " and a.luhid in (select org_luhid as luhid from  PDFTAG.dbo.GAP_Header where pipid = '" + pipid + "' )   \n";
            //Response.Write("<!--" + sSql + "-->");
            cm.CommandText = sSql;
            ...
            sSql += " and a.lustid in (select org_lustid as lustid from  PDFTAG.dbo.GAP_SizeTable where pipid = '" + pipid + "' )   \n";
            //Response.Write("<!--" + sSql + "-->");
            cm.CommandText = sSql;
