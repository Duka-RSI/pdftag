    ...
    public void saveByCol(HttpContext context)
    {
        ...
        using (var cn = SqlMapperUtil.GetOpenConnection("DB"))
        {
            ...
            if (col.StartsWith("A"))
            {
                ...
                sSql = "select lusthid  from  PDFTAG.dbo.GAP_SizeTable_1 where lustid_relation=@lustid";
                var resLu_SizeTable_1_Org = cn.Query<Text>(sSql, new { lustid = lustid_org }).FirstOrDefault();

                sSql = "select *  from  PDFTAG.dbo.GAP_SizeTable_Header_1  where lusthid=@lusthid";
